...
import gongolaLogo from "@/assets/gongola-logo.jpeg";
      ...
      <nav className="container flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <Link to="/" className="flex items-center">
            <img src={gongolaLogo} alt="Gongola Group Logo" className="h-12 w-auto" />
          </Link>
          <LanguageSelector />
        </div>
        ...
        <ul className="hidden md:flex space-x-8">
          {navLinks.map(link => {
            ...
            return <li key={link.name} className="relative">
              <Link to={link.path} className={cn("font-medium transition-colors hover:text-primary after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-primary after:transition-all hover:after:w-full", isActive ? "text-primary after:w-full" : "after:w-0")}>
                {link.name}
              </Link>
            </li>;
          })}
            ...
            <div>
              <div className="flex justify-between items-center mb-8">
                <Link to="/" onClick={() => setMobileMenuOpen(false)}>
                  <img src={gongolaLogo} alt="Gongola Group Logo" className="h-10 w-auto" />
                </Link>
              ...
              <ul className="space-y-6">
                {navLinks.map(link => {
                  ...
                  return <li key={link.name}>
                    <Link to={link.path} className={cn("text-lg font-medium transition-colors hover:text-primary", isActive && "text-primary font-semibold")} onClick={() => setMobileMenuOpen(false)}>
                      {link.name}
                    </Link>
                  </li>;
                })}
